...
OB_INLINE int ObTableDeleteOp::delete_row_to_das()
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < MY_SPEC.del_ctdefs_.count(); ++i) {
    ...
    for (int64_t j = 0; OB_SUCC(ret) && j < ctdefs.count(); ++j) {
      ...
      if (del_ctdef.das_ctdef_.use_row_cache_ && ctdefs.count() > 1 && del_ctdef.is_primary_index_) {
        ...
        extract_rowkey(tablet_loc, del_ctdef, rowkey);
...
int ObTableDeleteOp::extract_rowkey(ObDASTabletLoc *tablet_loc, const ObDelCtDef &del_ctdef, ObRowkey& rowkey) {
  ...
  const ExprFixedArray& row = del_ctdef.old_row_;
